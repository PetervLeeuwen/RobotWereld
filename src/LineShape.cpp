 ...
 *
 */
LineShape::LineShape(	ModelObjectPtr aModelObject,
...
std::string LineShape::asString() const
{
	...
	os << Shape::asString() << " LineShape: name = " << title << " " << Shape2DUtils::asString( getBegin()) << "->" << Shape2DUtils::asString( getEnd());
	return os.str();
}
/**
 *
...
std::string LineShape::asDebugString() const
{
	...
	os << "name = " << (title.size() == 0 ? "\"\"" : title) << ", " << Shape2DUtils::asString( getBegin()) << "->" << Shape2DUtils::asString( getEnd());
	return os.str();
}
